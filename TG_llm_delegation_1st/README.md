...
Trust Game (sequential) with **LLM chatbot**, and **`DELEGATION_FIRST=True`**.
...
| Config | Bots |
|--------|------|
| `TG_llm_delegation_1st` | No — **use this for Prolific** |
| `TG_llm_delegation_1st_with_bots` | Yes |

```bash
otree create_session TG_llm_delegation_1st 60
```
...
In this app Part 1 is agent-programmed and Part 2 is human. The matching / payoff logic is identical for every TG treatment; only **how** the A/B plans are produced differs (LLM chatbot).
