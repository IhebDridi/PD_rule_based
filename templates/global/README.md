...
Shared page classes in `pages_classes/` set `template_name` accordingly. **Exception:** `AgentProgramming` has no global template—each app keeps its own (e.g. `AgentProgramming.html`, `MistralPage.html`) under `<app>/templates/<AppName>/`.
