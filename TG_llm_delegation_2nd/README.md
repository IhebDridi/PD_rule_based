...
Trust Game (sequential) with **LLM chatbot**, and **`DELEGATION_FIRST=False`**.

> **Production note:** needs a live `MISTRAL_API_KEY` (and working assistant). Without it, participants can stall on the chat pages.
...
| Delegation UI | Chat with Mistral until it returns a strict 10-item A/B line per role. |
...
| Config | Bots |
|--------|------|
| `TG_llm_delegation_2nd` | No — **use this for Prolific** |
| `TG_llm_delegation_2nd_with_bots` | Yes |

```bash
otree create_session TG_llm_delegation_2nd 60
```
...
4. Agent: `TgLlmAgentFirst` / `TgLlmAgentSecond`
...
Pages: `TgLlmAgentFirst` / `TgLlmAgentSecond`.

Chat with Mistral until it returns a strict 10-item A/B line per role.

Saved / notable fields:

- `conversation_history` / `conversation_history_second` — full chat JSON
- Contingent choices copied into `choice_first_mover` / `choice_second_mover` after a valid plan

---

## Worked example: P1, P2, P3 in one trio

In this app Part 1 is human and Part 2 is agent-programmed. The matching / payoff logic is identical for every TG treatment; only **how** the A/B plans are produced differs (LLM chatbot).
...
| `mistralassistant.py` | LLM client helper |
