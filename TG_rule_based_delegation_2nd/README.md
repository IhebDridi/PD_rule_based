...
Trust Game (sequential) with **rule-based (manual A/B plan)**, and **`DELEGATION_FIRST=False`**.
...
| Config | Bots |
|--------|------|
| `TG_rule_based_delegation_2nd` | No — **use this for Prolific** |
| `TG_rule_based_delegation_2nd_with_bots` | Yes |

```bash
otree create_session TG_rule_based_delegation_2nd 60
```
...
In this app Part 1 is human and Part 2 is agent-programmed. The matching / payoff logic is identical for every TG treatment; only **how** the A/B plans are produced differs (rule-based (manual A/B plan)).
