...
# Patches get_rows_for_wide_csv_app if present (newer oTree), else wraps get_rows_for_wide_csv (older).
# -----------------------------------------------------------------------------
def _patch_wide_export():
    ...
    # Newer oTree: patch get_rows_for_wide_csv_app
    if hasattr(_exp, 'get_rows_for_wide_csv_app'):
        try:
            ...
            def _patched_app(app_name, max_round_number, sessions):
                ...
                for round_number in range(1, max_round_number + 1):
                    ...
                    header_row = []
                    for model_name, fields in [('player', pfields), ('group', gfields), ('subsession', sfields)]:
                        for fname in fields:
            ...
            _exp.get_rows_for_wide_csv_app = _patched_app
        except Exception:
            pass
        return

    # Older oTree (no get_rows_for_wide_csv_app): wrap get_rows_for_wide_csv to catch AssertionError
    if hasattr(_exp, 'get_rows_for_wide_csv'):
        _orig_wide = _exp.get_rows_for_wide_csv
        def _wrapped_wide(session_code):
            try:
                return _orig_wide(session_code)
            except AssertionError as e:
                if 'has 0 players' in str(e) or 'should always match' in str(e):
                    return []  # empty export so download doesn't 500
                raise
        _exp.get_rows_for_wide_csv = _wrapped_wide


# Defer patch to avoid circular import.
import threading
