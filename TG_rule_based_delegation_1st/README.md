# TG_rule_based_delegation_1st

Trust Game (sequential) with **rule-based (manual A/B plan)**, and **`DELEGATION_FIRST=True`**.
...
| Delegation UI | Participant manually enters A/B for all 10 rounds for each contingent role. |
...
| URL slug (`name_in_url`) | `exp_game411` |

Sibling app with reversed part order: [`TG_rule_based_delegation_2nd`](../TG_rule_based_delegation_2nd/).
...
| Config | Bots |
|--------|------|
| `TG_rule_based_delegation_1st` | No — **use this for Prolific** |
| `TG_rule_based_delegation_1st_with_bots` | Yes |

```bash
otree create_session TG_rule_based_delegation_1st 60
```
...
4. Agent: `TgV2AgentProgrammingFirst` / `TgV2AgentProgrammingSecond`
...
Pages: `TgV2AgentProgrammingFirst` / `TgV2AgentProgrammingSecond`.

Participant manually enters A/B for all 10 rounds for each contingent role.

Saved / notable fields:

- Agent form fields (`agent_decision_mandatory_delegation_round_*`, second-mover analogues)
...
In this app Part 1 is agent-programmed and Part 2 is human. The matching / payoff logic is identical for every TG treatment; only **how** the A/B plans are produced differs (rule-based (manual A/B plan)).
...
| `templates/TG_rule_based_delegation_1st/` | App-specific HTML |
