...
Trust Game (sequential) with **rule-based agent programming**, and **mandatory delegation in Part 1**.
...
| Config | Bots |
|--------|------|
| `TG_rule_based_delegation_1st` | No |
| `TG_rule_based_delegation_1st_with_bots` | Yes (`use_browser_bots=True`) |

Create a session from oTree Demo/Admin, or:
...
**Part 1** (mandatory rule-based agent) → BatchWait → Results →  
**Part 2** (human contingent decisions) → BatchWait → Results →  
**Part 3** (optional delegate) → BatchWait → Results →  
Guessing game → Debriefing → Exit Q → Thank you.

At the end of each part, `BatchWaitForGroup` forms **groups of 3** (FIFO) and computes payoffs from those three players only. Matching does **not** use oTree’s `group_id`.
